...
class UserProfile:
    ...
    def get_id(self) -> int:
        return extract_id(self.dom)

    def get_hq_status(self) -> bool:
        return extract_hq_status(self.dom)

    def get_display_name(self) -> str:
        return extract_display_name(self.dom)

    def get_bio(self) -> str | None:
        return extract_bio(self.dom)

    def get_location(self) -> str | None:
        return extract_location(self.dom)

    def get_website(self) -> str | None:
        return extract_website(self.dom)

    def get_watchlist_length(self) -> int | None:
        return extract_watchlist_length(self.dom)

    def get_stats(self) -> dict:
        return extract_stats(self.dom)

    def get_favorites(self) -> dict:
        return extract_favorites(self.dom)

    def get_avatar(self) -> dict | None:
        return extract_avatar(self.dom)

    def get_watchlist_recent(self) -> dict:
        return extract_watchlist_recent(self.dom)

    def get_diary_recent(self) -> dict:
        return extract_diary_recent(self.dom)
...
def extract_display_name(dom) -> str:
    ...
    try:
        ...
        if content:
            return content[
                :-10
            ]  # Remove the last 10 characters (e.g., " | Letterboxd")
        else:
    ...
    for section in sections:
        ...
        if section.h2.text == "Diary":
            ...
            for entry in entry_list:
                ...
                for day, item in zip(days, items, strict=False):
                    ...
                    diary_recent["months"][month_index][movie_index].append(
                        {"name": movie_name, "slug": movie_slug}
                    )
            break
    return diary_recent
