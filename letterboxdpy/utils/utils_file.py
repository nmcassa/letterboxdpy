...
class File:
    ...
    def save(path: str, data: dict, format: str = "json") -> None:
        ...
        if format == "json":
            ...
            raise ValueError(
                f"Unsupported format '{format}'. Only 'json' is currently supported."
            )
...
class JsonFile(File):
    ...
    @staticmethod
    def stringify(data, indent: int | None = None, encoder=None, **kwargs) -> str:
...
class CsvFile(File):
    ...
    @classmethod
    def save(cls, path: str, rows: list, headers: list | None = None) -> None:
