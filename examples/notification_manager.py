...
def display_menu(states, console):
    table = Table(
        title="Letterboxd Notifications", show_header=True, header_style="bold magenta"
    )
    ...
    for group_name, fields in NOTIFICATION_GROUPS.items():
        ...
        for label, key in fields:
            ...
            status_str = (
                "[bold green]ON[/bold green]" if status else "[bold red]OFF[/bold red]"
            )
...
def display_pending_changes(pending_changes: dict, console: Console):
    ...
    for key, val in pending_changes.items():
        ...
        label = key
        for fields in NOTIFICATION_GROUPS.values():
            for label_name, k in fields:
                if k == key:
                    label = label_name
                    break
...
def handle_save(
    settings: UserSettings, pending_changes: dict, console: Console
) -> list:
...
def handle_choice(
    ...
    if choice == "Q":
        return bool(
            pending_changes
            and not Confirm.ask("[yellow]Discard unsaved changes?[/yellow]")
        )
...
def main():
    ...
    parser.add_argument(
        "--path", type=Path, default=DEFAULT_COOKIE_PATH, help="Path to cookie file"
    )
    ...
    try:
        ...
        while True:
            ...
            choice = Prompt.ask(
                "→",
                choices=[str(i) for i in range(1, num_options + 1)] + ["S", "Q"],
                default="Q",
            ).upper()

            if not handle_choice(
                choice,
                states,
                original_states,
                pending_changes,
