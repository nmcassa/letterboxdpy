...
class App:
    ...
    def _download_poster(self, slug: str, file_path: str, count: int) -> None:
        ...
        if os.path.exists(file_path):
            if int(os.stat(file_path).st_size) == int(
                response.headers["Content-Length"]
            ):
                print(
                    f"{count} - File already exists and has same size as new file, skipping.."
                )
                return
    ...
    def _resolve_file_path(
        self, date: dict, slug: str, years_dir: str | None, previous_year: str | None
    ) -> tuple[str, str | None]:
    ...
    def run(self):
        ...
        for v in entries.values():
            ...
            if already_start and (already_start - count) > 1:
                print(
                    f"Have already processed {already_start - count} entries, skipping {count}.."
                )
                already_start = 0
